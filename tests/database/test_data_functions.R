...
testthat::test_that("get_tag_column_names returns NA when an empty data frame, NULL, or NA is passed.", {
  ...
  testthat::expect_that(get_tag_column_names(data_frame), is_identical_to(NA))
  testthat::expect_that(get_tag_column_names(NULL), is_identical_to(NA))
  testthat::expect_that(get_tag_column_names(NA), is_identical_to(NA))
})
