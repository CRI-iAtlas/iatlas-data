get_genes_by_type <- function() {
  ...
  get_genes <- function(gene_type) {
    ...
    # Get all the gene type ids related to the genes in the table.
    if (!is.na(gene_type)) {
      genes <- genes %>%
        dplyr::right_join(
    ...
    # Get all the related gene types from the gene_types table.
    genes <- genes %>%
      dplyr::left_join(
    ...
    # Filter the genese down to genes with only the passed gene type.
    if (!is.na(gene_type)) {
    ...
    # # Get all the related gene families from the gene_families table.
    genes <- genes %>%
      dplyr::left_join(
    ...
    # Get all the related gene funtions from the gene_functions table.
    genes <- genes %>%
      dplyr::left_join(
    ...
    # Get all the related immune checkpoints from the immune_checkpoints table.
    genes <- genes %>%
      dplyr::left_join(
    ...
    # Get all the related pathways from the pathways table.
    genes <- genes %>%
      dplyr::left_join(
    ...
    # Get all the related super categories from the super_categories table.
    genes <- genes %>%
      dplyr::left_join(
    ...
    # Get all the related therapy types from the therapy_types table.
    genes <- genes %>%
      dplyr::left_join(
